...
The current local JSON exporter persists a versioned per-file scan index in plugin `data.json`. Top-level `vaultId`, `timeZone`, `defaultAlertTime`, and `ntfyServer` settings remain readable, while the private version 6 persistence format uses compact tuples, a deduplicated configuration-fingerprint table, and unpadded base64url SHA-256 values. Per-file reminder diagnostics and pending local-write and server-POST states are persisted with the scan results. `ntfyServer` starts blank and should contain a full HTTP(S) topic URL such as `https://ntfy.sh/your-topic`. Optional authentication is supplied in the topic URL using the `auth` query parameter; the full URL is persisted and exported unchanged. Fingerprints and paths are sorted for deterministic bytes, and the complete file is minified without a trailing newline. Missing, corrupt, or incompatible index data causes a safe full scan without discarding valid top-level settings.
...
`reminders.json` is a JSON object with the configured ntfy topic URL in the top-level `ntfy-server` property, followed by a key for the vault name and then vault-relative Markdown paths. Each file maps to a line-sorted reminder array, allowing multiple tasks in one note without repeating the vault or file strings. A reminder contains its one-based `line` number instead of a composite ID because its vault and file are implied by the enclosing keys. Each reminder also contains `text`, the trimmed description before the bell with the Tasks Global Filter removed, and optional `priority` (`highest`, `high`, `medium`, `low`, or `lowest`) from the Tasks priority emoji. Markdown and other tags in the description are preserved. The checkbox, reminder expression, and trailing Tasks metadata are excluded. These fields are persisted in the compact cache; older caches are rescanned to populate them. The file keys and reminder arrays are sorted, and the output uses two-space indentation with a trailing newline. The exporter builds it from cached per-file results and compares exact bytes with the last successfully read or written output. Startup, settings changes, and foreground reconciliation repair missing, malformed, or externally altered output; unchanged bytes are never written. Index state is persisted before changed output, and a persistent retry flag defers failed writes until the next event, reconciliation, or manual regeneration.
